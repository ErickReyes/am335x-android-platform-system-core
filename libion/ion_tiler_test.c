		    ...
		    struct omap_ion_tiler_alloc_data *al_data)
{
	...
	ret = ion_alloc_tiler(fd, al_data->w, al_data->h, al_data->fmt, al_data->flags, handle, &stride);

	printf("stride %d\n ", stride);

	al_data->stride = stride;
...
int alloc_1D_test(uint32_t length, size_t stride)
{
	...
	fflush(stdout);
		...
		printf("\ncalling ion alloc");	fflush(stdout);

	ret = _ion_alloc_test(fd, &handle, &alloc_data);
	if (ret) {
		printf("\nion_alloc_test failed!! with ret=%d", ret);	fflush(stdout);\
		exit(-1);
		goto exit;
	}
	...
	printf("\ncalling ion map");	fflush(stdout);


	ret = ion_map(fd, handle, length, prot, map_flags, 0, &ptr, &map_fd);
        if (ret) {
			printf("\n ION_MAP failed!! %d", ret);
		return ret;
        	}

	
	printf("\ncalling fill mem");	fflush(stdout);

	fill_mem(val, ptr, &alloc_data);
	ret  = check_mem(val, ptr, &alloc_data);
	if (ret) {
		printf("\n%s: memory validation failed\n", __func__);	fflush(stdout);
		munmap(ptr, length);
		goto exit;
	}

	
	printf("\ncalling unmap");	fflush(stdout);

	munmap(ptr, length);
	
	printf("\ncalling ion_free");	fflush(stdout);

	ret = ion_free(fd, handle);
	if (ret) {
		printf("\n%s failed: %s %p\n", __func__, strerror(ret), handle);	fflush(stdout);
		return ret;
	}

		
	printf("\ncalling close");	fflush(stdout);
	close(map_fd);
exit:
	ion_close(fd);
	return ret;
